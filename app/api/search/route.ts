...
export async function GET(requestUrl: NextRequest) {
  ...
  if (type === "price") {
    ...
    upstreamPath = `${requiredEnv("MARKET_PATH_SEARCH_PRICE")}?date=${date}&price=${price}&filter=${filter}`;
  } else if (type === "profit") {
    if (!/^\d{8}$/.test(date)) return NextResponse.json({ message: "날짜는 YYYYMMDD 형식이어야 합니다." }, { status: 400 });
    upstreamPath = `${requiredEnv("MARKET_PATH_SEARCH_PROFIT")}?date=${date}`;
  } else if (type === "have") {
    ...
    upstreamPath = `${requiredEnv("MARKET_PATH_SEARCH_HOLDING")}?datebegin=${datebegin}&dateend=${dateend}&investors=${investors}`;
  } else {
